...
const OrderSec = ({ order }) => {
    ...
    const makePayment = async (id) => {
        ...
        let result = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/order/payment`, {
            method: "PUT",
            body: JSON.stringify({ orderid: id })
        }).then(response => response.json());
                    ...
                    <div className='flex items-start justify-between flex-col-reverse md:flex-row gap-5 text-center md:text-left'>
                        <div className='max-w-full md:max-w-[48%] w-full'>
                            <div className="pb-[30px] border-b border-grey">
                                ...
                                {order?.isComplete === 1 &&
                                    <>
                                        <h2 className='section-secondary-heading mb-[30px]'>Pending receipt</h2>
                                        <div className='flex items-center gap-[20px]'>
                                            <p className='info-14 max-w-[50%] w-full hover:!text-grey'>seller will release asset in <span className='font-bold text-primary'>{timeLeft}</span> minutes.</p>
                                            <div className='flex items-center gap-[20px] grow max-w-[50%] w-full '>
                                                <p className='info-14-24 font-bold !text-primary flex grow justify-end'>{order != undefined && order?.spend_amount} <span>&nbsp;INR</span></p>
                                            </div>
                                        </div>
                            ...
                            {order?.inProcess === 1 &&
                                <>
                                    <button className="cta w-full mt-[24px]" onClick={() => { setClick(true); setOrderPopup(true) }}>Paid <span></span></button>
                            ...
                            {order?.isComplete === 1 &&
                                <>
                                    <button className="cta w-full mt-[24px]" >Releasing <span></span></button>
