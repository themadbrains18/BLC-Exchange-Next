...
const Paymentmodal = ({paymentods}) => {
  ...
  const onSubmit = async (data) => {
    ...
    if(flag){
      ...
      const addNew = await fetch(`${process.env.NEXT_PUBLIC_BASEURL}/payment`,{ method : "POST", body : JSON.stringify(data)})
  ...
  return (
    <div>
      ...
      <div className={`${popup ? "opacity-1 visible fixed top-[50%]":"opacity-0 invisible top-[55%]" }  duration-300 z-[20] fixed  left-[50%] translate-y-[-50%] w-[calc(100%-20px)] sm:w-full translate-x-[-50%] dark:bg-black-v-5 bg-white p-3 sm:p-6 border border-grey max-w-[480px] w-full mx-auto`}>
                            ...
                            <p className="info-16-22 dark:!text-white !text-black mt-[15px] text-center text-lg">Add payment method</p>
...

                            {
                              paymentods[defaultVl].fields.map((field, index) => {
                                return (
                                  <div key={index}>
                                    <Input name={field.name} label={field.label} register={register} required={field.required} type={field.type} errors={errors}  />
                                    <div className="!text-red-700 info-12">
                            ...
                            <div className="">
                              <div className="flex justify-between">
                                  <label className=" dark:!text-white !text-black  text-center"><span className='text-[#f00]'>*</span> Fund Code</label>
                                  <Link href="#" className=" dark:!text-white !text-black  text-center">Forgot your fund code?</Link>
                              </div>
                            ...
                            <div className="flex justify-between mt-4">
                                <button className=" dark:!text-white !text-black  text-center py-2 px-10 border border-[#b0a4b366] rounded-lg ">Cancel</button>
                                <button className=" dark:!text-white !text-black  text-center py-2 px-10 border border-[#b0a4b366] rounded-lg bg-primary">Save and use</button>
                            </div>
